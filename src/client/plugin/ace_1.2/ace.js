...
ace.handle_side_menu = function($) {
	...
	$('.nav-list').on(ace.click_event, function(e){
		...
		if(! link_element.hasClass('dropdown-toggle') ) {//it doesn't have a submenu return
            ...
            if(ace.displayParent != null && ace.displayParent.length > 0){
                for(var i = 0; i < ace.displayParent.length; i++){
                    $(ace.displayParent[i]).css("display", "");
                }
                ace.displayParent = [];
            }
			return;
        ...
        if(ace.displayParent == null){
            ace.displayParent = [];
        }
        if(ace.displayParent.indexOf(sub) === -1){
            ace.displayParent.push(sub);
        }
		return false;
	 })
}
